...
async function runAs(callback) {
  ...
  try {
    ...
    if (rows.length === 0) throw new Error(`No active session for user ${userId}`);
...
function score(text) {
  ...
  const hasWarning = /^\s*\*\*(Warning|Important|Note|Caution|Disclaimer)\b/im.test(text);
  const hasNumbered = /(^|\n)\s*\d+[.)]\s+[A-Z]/.test(text);
  const hasSummary = /\b(Key Takeaways|In Summary|To Summarize|Conclusion)\b/i.test(text);
  let format = "plain";
  ...
  const sectionsCited = (text.match(/(?:section|sec\.)\s+\d+[a-zA-Z]?/gi) || []).length;
...
function verdict(t, s, text) {
  ...
  if (t.expect === "warning" && !s.hasWarning) {
    if (!/penalt|fine|prosecut|punish|jail|consequen/.test(low)) problems.push("risk reply missing warning language");
  }
  if (t.expect === "greeting" && !/hello|hi|how can i assist/i.test(low)) problems.push("expected a greeting");
  if (t.expect === "indian-only-refusal" && !/only provide information related to indian law/i.test(low)) {
    problems.push("did not refuse non-Indian query");
  }
  if (t.expect === "correct-bns-302") {
    if (/indian penal code|ipc\b/.test(text)) {
      if (!/religious|wound|feeling/i.test(text)) problems.push("answered with IPC 302 (murder) not BNS 302");
    }
  }
  ...
  if (t.expect === "blank-template") {
    if (!/\[[a-z ]+\]/i.test(text) || text.trim().length < 300) problems.push("not a full blank template");
  }
...
await runAs(async (cookie) => {
  ...
  for (const t of TESTS) {
    ...
    process.stdout.write(`# ${t.id} ${t.mode.padEnd(8)} "${t.q.slice(0, 60)}..."\n`);
  ...
  out.push("");
  out.push("Legend: w=words, ASL=avg sentence length, FRE=Flesch Reading Ease (90+=very easy, 60-70=plain, <40=hard), FK=Flesch-Kincaid grade, Jargon=% legal terms. Score 1-5 stars.");
  out.push("");
  out.push("| ID | Mode | Question | Size | ASL | FRE | FK | Jargon | Format | Score | Issues |");
  out.push("|----|------|----------|------|-----|-----|----|--------|--------|-------|--------|");
  for (const r of results) {
    if (r.status !== 200) {
      out.push(`| ${r.id} | ${r.mode} | ${r.q.replace(/\|/g, "\\|")} | **ERR ${r.status}** | - | - | - | - | ${(r.error || "").slice(0, 40)} |`);
      continue;
    }
    out.push(`| ${r.id} | ${r.mode} | ${r.q.replace(/\|/g, "\\|")} | ${r.s.words}w | ${r.s.asl} | ${r.s.fre} | ${r.s.fk} | ${r.s.jargonPct}% | ${r.s.format} | ${"★".repeat(r.v.score)}${"☆".repeat(5 - r.v.score)} | ${r.v.problems.join("; ")} |`);
  }
  ...
  for (const r of results) {
    ...
    out.push(`- Words: ${r.s.words} | Sentences: ${r.s.sentences} | ASL: ${r.s.asl} | FRE: ${r.s.fre} | FK: ${r.s.fk}`);
    out.push(`- Jargon: ${r.s.jargonPct}% | Format: ${r.s.format} | Sections cited: ${r.s.sectionsCited}`);
    out.push(`- Score: ${"★".repeat(r.v.score)}${"☆".repeat(5 - r.v.score)}`);
    if (r.v.problems.length) out.push(`- Problems: ${r.v.problems.join(" | ")}`);
  ...
  fs.writeFileSync(path.join(ROOT, "test-results", "chat-eval-report.json"), JSON.stringify(results, null, 2));
